...
- Bad, because Ruralz aggregates duplicate part of the SDK pipeline and forgo its exemplar reservoirs; their export interface is OQ-observability-16 (blocking for M1 metrics), whose option (c) amends this ADR.
...
- Bad, because, with `OTEL_*` removed, OTLP TLS and authentication settings wait on OQ-observability-2 (blocking for TLS export at M1); cleartext raises `cleartext_hop`.
...
- **Blocked stdout test**, Planned (M1): with stdout blocked, request latency stays within [Overhead per signal](../architecture/10-observability.md#overhead-per-signal) budgets (target) and drops are counted.
...
- **Audit export test**, Planned (M2): across a leader failover, against a Collector returning partial success, every committed audit entry arrives at least once, carrying the Resource-row attributes.
