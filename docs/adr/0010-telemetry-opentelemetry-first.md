...
Ruralz Gateway and Ruralz Control emit metrics, traces and logs, and P10 requires OpenTelemetry signals in every build ([Vision and positioning](../vision/01-vision-and-positioning.md#principles)). Operators set only `Gateway.spec.telemetry.otlp.endpoint`, `traceSampling` and `accessLog.when` ([Configuration model](../architecture/02-configuration-model.md#gateway)).

Which layer should every package code against, when OpenTelemetry Go traces and metrics are stable but Logs is a release candidate ([source](https://github.com/open-telemetry/opentelemetry-go/blob/main/README.md))? Ruralz Gateway telemetry is Planned (M1), Ruralz Control Planned (M2), AI Planned (M3).

## Decision drivers

- **Vendor-neutral and free (P1, P10)**: identical signals in every build, the FIPS build (Planned (M5)) and air-gapped included.
- **Never on the critical path**: no request goroutine waits on a lock, pipe or exporter; bounded queues drop with a counter (rule O2 of [Observability](../architecture/10-observability.md#observability-principles); P3's "bounded and droppable").
...
- **One correlation key (O6)**: (`trace_id`, server `span_id`) links spans, logs and exemplars; problem-document `requestId` ([Data plane](../architecture/03-data-plane.md#error-response-format)) carries the trace ID, the span ID being OQ-observability-13.
...
2. **OpenTelemetry for all three signals now**, coding logs against the Logs API of v1.47.0-rc.1 ([source](https://github.com/open-telemetry/opentelemetry-go/releases/tag/v1.47.0-rc.1)).
...
Chosen option: "OpenTelemetry-first with an slog bridge", because OTLP carries all three signals while every call site uses a stable API (OpenTelemetry traces and metrics, `log/slog`), per foundation pack section 7.

End state: when v1.47.0 ships the stable Logs API and SDK, as expected ([source](https://github.com/open-telemetry/opentelemetry-go/releases/tag/v1.47.0-rc.1)), they replace the release-candidate SDK behind the bridge; call sites stay on `log/slog`, and `otelslog` stays in `internal/telemetry` as the `slog`-to-Logs-API adapter. "Until" in pack section 7 means the pre-stable SDK; the [Watch list](../engineering/01-tech-stack-and-libraries.md#watch-list) trigger changes only wiring.

| Area | Rule | Planned |
|---|---|---|
| Traces | SDK tracer; W3C Trace Context extracted at the server span, injected into every upstream leg; no baggage. `internal/telemetry` samples under both per-Node caps before calling the SDK, so unsampled requests create no spans, and installs a lock-free per-connection ID generator (`sdktrace.WithIDGenerator`) | Planned (M1) for HTTP; Planned (M3) for gRPC; Planned (M4) for Kafka, NATS and MQTT 5 |
| Metrics | Ruralz-owned aggregates, not SDK synchronous instruments, read by OTLP and `/metrics` through the SDK interface of OQ-observability-16 | Planned (M1) |
| Exemplars | Only sampled requests write the one slot per histogram label set per collection, under a sequence lock off the unsampled path; exported through OQ-observability-16 and OpenMetrics on `/metrics`; slot memory adds to the 300-byte histogram assumption (hypothesis) | Planned (M1) |
| Logs | Only `log/slog` via `internal/telemetry`, except the audit export; no request goroutine runs a writing handler. Access log fields go into pooled records in a bounded queue of 8,192 records and 4 MiB (target); one worker encodes each once for stdout and the bridge. Process logs use a non-blocking handler enqueueing into a bounded queue drained by the one goroutine owning stdout. Both drop with `ruralz_telemetry_logs_dropped_total{stream, reason="queue_full"}`. Raft's `go-hclog` is adapted to `slog` ([ADR-0006](0006-control-store-raft-boltdb.md)) | Planned (M1); Ruralz Control Planned (M2) |
| Export | Node OTLP only when `Gateway.spec.telemetry.otlp.endpoint` is set, over TLS (TB-12); spans and logs, except the audit export, pass queues dropping with `queue_full` or `export_error` counts; failure past one interval raises `telemetry_export_failing`. Providers, processors and exporters get explicit options and the resource no environment detector, so `OTEL_*` process variables are ignored; `${OTEL_...}` render-time substitution stays valid | Planned (M1) |
| Audit export | The Ruralz Control leader calls the OTLP log exporter directly, bypassing `slog` and the drop queues: batches keyed by Raft index, retried until acknowledged, partial success counted as failure; it lags, never drops (TB-12 exception proposed in OQ-observability-19) | Planned (M2) |
...
*Figure 1: signal paths from a Node or Ruralz Control, which serves `/metrics` on 9902; the dashed edge is the audit export.*
...
- Good, because one OTLP pipeline reaches any store, with no Ruralz code per vendor (P1).
- Good, because `slog` call sites survive v1.47.0 unchanged.
- Good, because (`trace_id`, server `span_id`) correlates spans, access log records, exemplars and `requestId` (O6).
...
- Bad, because the audit export, which must not lose records, uses the pre-stable OTLP log exporter and waits on OQ-observability-19 (blocking for Planned (M2)) to amend TB-12's "Drop on failure".
...
- Bad, because Ruralz aggregates duplicate part of the SDK pipeline and forgo its exemplar reservoirs; their export interface is OQ-observability-16 (blocking for M1 metrics), whose option (c) would amend this ADR.
...
- Bad, because with `OTEL_*` ignored, OTLP TLS settings and authentication wait on OQ-observability-2 (blocking for TLS export at M1); cleartext raises `cleartext_hop`.
...
- **Lint**, Planned (M0): depguard `log$` rejects `log` ([Banned imports](../engineering/02-repository-layout-and-conventions.md#banned-imports)); `sloglint` checks `slog` keys, `spancheck` unended spans ([golangci-lint linter set](../engineering/02-repository-layout-and-conventions.md#golangci-lint-linter-set)); repocheck checks names against pack section 2 ([Ruralz-specific checks](../engineering/02-repository-layout-and-conventions.md#ruralz-specific-checks)).
- **Catalog gates**, Planned (M1): CI fails on a `ruralz_*` name or degraded reason absent from the [Metrics catalog](../architecture/10-observability.md#metrics-catalog), a Grafana panel on an unlisted metric, or OTLP and `/metrics` names that differ.
...
- **Blocked stdout test**, Planned (M1): with stdout blocked, request latency stays within [Overhead per signal](../architecture/10-observability.md#overhead-per-signal) budgets (target) while drops are counted.
- **Environment golden test**, Planned (M1): with `OTEL_RESOURCE_ATTRIBUTES`, `OTEL_EXPORTER_OTLP_HEADERS` and `OTEL_TRACES_SAMPLER` set, `ruralzd` exports a byte-identical resource, headers and sampling.
...
- **Audit export test**, Planned (M2): across a leader failover, against a Collector returning partial success, every committed audit entry arrives at least once.
...
- Revisit at OpenTelemetry Go v1.47.0, or when OQ-observability-16 picks Ruralz encoders, OQ-observability-2 adds TLS fields or `gen_ai` tags a release.
- Proposed amendments: tech stack Watch list Fallback for the OTel Logs API: "Stable Logs SDK behind the `otelslog` bridge; `slog` call sites unchanged"; pack section 7: "`slog` bridge (`otelslog`) for logs, over a pre-stable Logs SDK until OTel Go v1.47.0"; Repository layout and conventions (owner of depguard rules): confine `go.opentelemetry.io/otel`, OTLP exporter modules and `otelslog` to `internal/telemetry` (S7), so the checklist item becomes lint.
