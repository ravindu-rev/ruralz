...
Which stack carries these protocols in a `CGO_ENABLED=0` binary ([ADR-0001](0001-implementation-language-go.md)), keeps every stream, buffer and deadline bounded, and still offers HTTP/3, which KrakenD lacks ([source](https://www.krakend.io/features/))? Nothing is implemented yet: HTTP/1.1, HTTP/2 and h2c are Planned (M1), HTTP/3 Planned (M3).
...
| HTTP/2 settings | Only `HTTP2Config`; on client listeners `MaxConcurrentStreams`, `MaxReceiveBufferPerConnection`, `MaxReceiveBufferPerStream` and `MaxReadFrameSize`, set once at start. A Hot Reload that changes a listener builds a new socket and server before the swap | Planned (M1) |
...
| HTTP/3 | quic-go `http3` v0.63.x, pinned, behind an internal interface, on UDP at the `https` port when `http3: true`. QUIC connections count against the Node connection ceiling and streams against the in-flight ceiling; quic-go stream, receive-window and header limits stay at or below the HTTP/2 values (target), within the 592 KiB per-connection budget (hypothesis), with deadlines through `http.ResponseController`. The quic-go settings and deadline support await research. 0-RTT stays off ([Security and identity](../architecture/08-security-and-identity.md)) | Planned (M3) |
...
*Figure 1: the stack per listener and upstream leg; dashed edges are Planned (M3); native gRPC and WebSocket stay on TCP.*
...
- Bad, because `ruralzd` links two HTTP/2 clients, `http.Transport` and grpc-go's transport for `grpc` Upstreams ([Library catalog](../engineering/01-tech-stack-and-libraries.md#library-catalog)), so client limits are set in two places.
- Bad, because in-flight QUIC connections on UDP 8443 are lost at a Zero-Downtime Upgrade, or when a Hot Reload replaces the UDP socket, while TCP connections Drain ([ADR-0015](0015-zero-downtime-upgrades-so-reuseport.md), OQ-system-overview-18).
...
- **Protocol conformance suite** in `pr-full`, in floor and release jobs: HTTP/1.1 and HTTP/2 cases, Planned (M1), and HTTP/3 cases, Planned (M3), that assert the QUIC stream cap and per-connection memory against the data plane budget; frame-level suites wait on OQ-testing-and-quality-strategy-6 ([Conformance suites](../engineering/03-testing-and-quality-strategy.md#conformance-suites)).
...
- Proposed owning-document amendments:
  - Data plane SHOULD add HTTP/3 Bounded resources rows, an Open question on the quic-go limits and `http.ResponseController` deadline support, `StrictMaxConcurrentRequests` false in Upstream layer, and an Open question on HTTP/3 discovery (`Alt-Svc`), which the M3 HTTP/3 row depends on.
