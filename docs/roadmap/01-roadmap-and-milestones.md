...
This document orders milestones `M0` to `M5` without dates, giving each scope, measurable exit criteria and KrakenD Enterprise Edition (EE) parity, plus sequencing, risks and contribution areas. Nothing is implemented. Readers: evaluators and contributors.
...
Parity is the cumulative share of the 71 EE-only rows in the 2026-09-23 KrakenD snapshot ([source](https://www.krakend.io/features/)) implemented at a milestone's exit: rows an architecture document maps count there, the rest are allocated here provisionally (OQ-roadmap-and-milestones-1). Not planned: Lua advanced helpers, NTLM authentication, New Relic native SDK.
...
| Security | `auth.jwt` (JWT, OpenID Connect, OAuth2; JOSE and OIDC libraries), `auth.api-key` (removal in file mode), `auth.basic`, `auth.mtls`, `authz.cel` (Security Policies Engine, first part), `authz.ip`, `auth.upstream-oauth2` (client credentials), `cors`; TLS to IdP JWKS endpoint (TB-10), Upstreams, State Store, OTLP (TB-3, TB-4, TB-12; `AIProvider` from M3); digest checks on every Revision and Plugin artifact; redaction unit tests; secret leak tests |
| Traffic and state | `ratelimit` with local token bucket and GCRA ([ADR-0008](../adr/0008-rate-limiting-local-bucket-and-gcra.md)), distributed rate limiting, KrakenD row "Stateful rate limit (Redis backed)", `quota` in `requests`, `cache` (Response Cache), retries, circuit breaker, health checks, catch-all fallback, header and query string routing, wildcard routes, service discovery (`dns`, static `endpoints`) for `http` Upstreams, weighted splits, composition; `memory` and `redis` drivers (tested on Valkey 9.0.1 or newer, Dragonfly), single-Region Cells; `headers`, `transform.request`, `transform.response`, `validation.json-schema` |
...
7. Every Open question blocking an M1 feature is closed: the 30 whose Blocking column names M1, plus OQ-configuration-model-8 and OQ-security-and-identity-2, -3 and -15 (`auth.basic`, `auth.mtls`), among them OQ-security-and-identity-21, -22, -24, OQ-repository-layout-and-conventions-6 and OQ-release-versioning-and-compatibility-3; OQ-traffic-management-and-resilience-5 and OQ-scalability-and-distributed-state-3 close only their M1 parts (breaker `minimumLegs`; Response Cache).

KrakenD EE parity at exit: 23 of 71, 32% (target). Mapped: extended flexible configuration, dump to disk, catch-all, header and query string routing, conditional routing, wildcard routes, URL rewrite, virtual hosts, basic authentication, API keys, multiple identity providers, customizable circuit breaker, service, tiered and stateful rate limits, IP filtering. Provisional: request body extractor, request and response template manipulation, response query language and regular expression replacements (`transform.*`), custom access log, linear workflows (OQ-data-plane-3).
...
| Traffic | Built-in client redirects (OQ-traffic-management-and-resilience-13); `kubernetes` discovery |
...
6. Every Open question blocking an M2 feature is closed: the 47 whose Blocking column names M2 at this snapshot, plus OQ-tech-stack-and-libraries-24 (`authz.geoip`), OQ-security-and-identity-10 (`vault`) and -28 (`auth.upstream-sigv4`), among them OQ-control-plane-and-gitops-1, -13, -25, OQ-security-and-identity-4 (revocation list), OQ-traffic-management-and-resilience-13 (redirects) and OQ-observability-19.

KrakenD EE parity at exit: 35 of 71, 49% (target): plugin generator, end-to-end testing tool, OpenAPI importer and exporter, Postman and DOT generators, Security Policies Engine, client redirects, Revoke Server, GCP and SigV4 authentication, MaxMind GeoIP.
...
5. Chaos tests pass the LLM provider throttling failure row, with CE-13 and AI provider mock 429 case.
6. The Open questions blocking M3 features are closed: 12 in AI/LLM gateway, 10 in Multi-protocol, OQ-tech-stack-and-libraries-21, OQ-system-overview-5 with OQ-multi-protocol-3, OQ-scalability-and-distributed-state-3 (Semantic Cache part), OQ-testing-and-quality-strategy-6 and -7, and OQ-deployment-topologies-13 (conversion webhook port, pack 8.4 amendment).

Measured and published at exit, not gating: SM-14 (hypothesis).

KrakenD EE parity at exit: 53 of 71, 75% (target): all 11 AI Gateway rows, MCP Server, streaming and SSE, gRPC server and client, direct WebSockets, WebSockets multiplexer, token quota enforcement.
...
| Bench suite | Competitor baselines for KrakenD CE, Envoy, Apache APISIX and Kong on RH-1 (F-7); S8; `default.pgo` |
...
KrakenD EE parity at exit: 55 of 71, 77% (target): Kafka async agents, advanced Apache Kafka.
...
KrakenD EE parity at exit: 68 of 71, 96% (target), or at least 61 of 71, 86% (target), if SM-3 uses all 10 Not planned slots. Rows: FIPS-140-2 module, API monetization and the 11 long-tail rows.
...
|---|---|---|---|---|
| OQ-roadmap-and-milestones-1 | Do the provisional parity allocations hold (M1 transform, access log and workflow rows; 11 long-tail rows at M5)? | (a) Adopt in the parity matrix; (b) move rows one by one; (c) mark some Not planned within SM-3's limit | krakend-ee-parity-matrix | Yes, for M1 exit criterion 6 |
