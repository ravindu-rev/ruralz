...
| backend `encoding` `xml`, `rss`, `string`, `yaml` | None | `manual` | CEL sees bodies only as JSON (OQ-data-plane-17) <!-- alias-ok --> |
...
| `auth/aws-sigv4` | B | `auth.upstream-sigv4` on the `Upstream`, Planned (M2) | `manual` | Region and service map to `config.region` and `config.service`; other KrakenD fields are unresearched ([source](https://www.krakend.io/docs/enterprise/authentication/aws-sigv4/)) <!-- alias-ok --> |
...
| `backend/soap` | B | `transform.request` XML envelopes, Planned (M5) | `manual` | XML responses per OQ-data-plane-17 ([source](https://www.krakend.io/docs/enterprise/backends/soap/)) <!-- alias-ok --> |
...
| SOAP integration | `backend/soap` is `manual`; CEL cannot read XML responses <!-- alias-ok --> | OQ-data-plane-17 |
