...
| Spike arrest and burst | Yes | Yes | Planned (M1) | `ratelimit` `limits[]` with a short `window` | `limits[].burst` ([Per-Node ceiling](../architecture/09-traffic-management-and-resilience.md#per-node-ceiling)), authored in Traffic management and pending registration (OQ-traffic-management-and-resilience-1) |
...
| Service rate limit | No | Yes | Planned (M1) | Gateway-scoped `ratelimit` with a constant `config.key` | Partial parity: a constant key is one State Store hot key: a limit above about 25,000 requests per second (hypothesis) needs a declared `limits[].perNodeCeiling` or `config.localOnly` ([Per-Node ceiling](../architecture/09-traffic-management-and-resilience.md#per-node-ceiling)), authored in Traffic management and pending registration (OQ-traffic-management-and-resilience-1), the latter also pending the pack 8.8 amendment, OQ-scalability-and-distributed-state-11 (blocking M1) ([Distributed rate limits](../architecture/11-scalability-and-distributed-state.md#distributed-rate-limits)) ([source](https://www.krakend.io/docs/enterprise/service-settings/service-rate-limit/)) |
...
| Proxy rate limit | Yes | Yes | Planned (M1) | `ratelimit` with a constant key on each `Route` reaching the `Upstream`; `circuitBreaker.maxPendingRequests` | Partial parity: Upstream-scoped `ratelimit` per OQ-krakend-ee-parity-matrix-4. A constant key is one State Store hot key: a limit above about 25,000 requests per second (hypothesis) needs a declared `limits[].perNodeCeiling` or `config.localOnly`, authored in Traffic management and pending registration (OQ-traffic-management-and-resilience-1), the latter also pending the pack 8.8 amendment, OQ-scalability-and-distributed-state-11 (blocking M1) |
