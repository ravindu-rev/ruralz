...
| Flatmap | Yes | Yes | Planned (M1) | `transform.response` `arrayOps[]` array operations | Pending registration ([source](https://www.krakend.io/docs/backends/flatmap/)) |
...
| Request body extractor | No | Yes | Planned (M1) | `transform.request` `set[]` copies body fields into headers or the query string | New in EE 2.13 ([source](https://www.krakend.io/docs/enterprise/endpoints/request-body-extractor/)); pending registration |
| Request manipulation using Go templates | No | Yes | Planned (M1) | `transform.request` `body` builds the Upstream body with CEL | No Go templates ([source](https://www.krakend.io/docs/enterprise/backends/body-generator/)); pending registration |
| Response manipulation using Go templates | No | Yes | Planned (M1) | `transform.response` `body` builds the client body with CEL | ([source](https://www.krakend.io/docs/enterprise/backends/response-body-generator/)); pending registration |
| Response manipulation with query language | No | Yes | Planned (M1) | `transform.response` `body` or `set[]` CEL expressions over `response.body` | CEL replaces JMESPath ([source](https://www.krakend.io/docs/enterprise/endpoints/jmespath/)) |
| Regular expression replacements | No | Yes | Planned (M1) | `transform.response` `replace[]`, literal or regular expression | ([source](https://www.krakend.io/docs/enterprise/endpoints/content-replacer/)); pending registration |
...
| Basic authentication | No | Yes | Planned (M1) | `auth.basic` binding a `Consumer` through `credentials.basic` | PBKDF2 hashes; [Registered from feature documents](../architecture/02-configuration-model.md#registered-from-feature-documents) |
...
| Revoke Server | No | Yes | Planned (M2) | Ruralz Control revocation API feeding that list, enforced by `auth.jwt` | File-mode delivery; `/api/v1/revocations` ([CLI and API surface](../reference/01-cli-and-api-surface.md#rest-api-on-8090)) awaits OQ-security-and-identity-4 |
...
| mTLS | Yes | Yes | Planned (M1) | `auth.mtls` `config.caCertificate` with `Consumer` `credentials.certificates` for clients; `Upstream.spec.tls` `clientCertificate` toward Upstreams | [Registered from feature documents](../architecture/02-configuration-model.md#registered-from-feature-documents) |
...
| Google GCP authentication | No | Yes | Planned (M2) | JWT-bearer grant in `auth.upstream-oauth2` | `grantType: jwt-bearer`, `serviceAccountKey`, `audience` ([Upstream authentication](../architecture/08-security-and-identity.md#upstream-authentication)), pending registration (OQ-security-and-identity-12) ([source](https://www.krakend.io/docs/enterprise/authentication/gcloud/)) |
...
| Unified LLM interface and prompt templates | No | Yes | Planned (M3) | `ai.surface: openai` façade; prompts shaped by `transform.request` `body` or a `plugin` Policy | KrakenD uses Go templates ([source](https://www.krakend.io/docs/enterprise/ai-gateway/unified-llm-interface/)); [Transform Policies](../architecture/03-data-plane.md#transform-policies) |
...
| SOAP integration | No | Yes | Planned (M5) | `transform.request` `body` builds XML envelopes with CEL string functions and `contentType: text/xml` for an `http` `Upstream`; `transform.response` reads them once an XML decoder exists | Partial parity: CEL can build the XML request, but reading an XML response needs an XML decoder for `response.body`, which the Configuration model lacks (OQ-data-plane-17); meanwhile a `plugin` Policy ([source](https://www.krakend.io/docs/enterprise/backends/soap/)) |
| WebSockets multiplexer | No | Yes | Planned (M3) | `Upstream` `protocol: websocket` with one shared connection per Route and Endpoint per Node | Envelope `ruralz.mux.v1` ([Multiplexing](../architecture/07-multi-protocol.md#multiplexing)); settings per OQ-multi-protocol-6; the shared reader never pauses, so an overflowing session closes alone with 1013 ([source](https://www.krakend.io/docs/enterprise/websockets/)) |
...
| Azure Service Bus topic and subscription | Yes | Yes | Not planned | No matching value in `Upstream.spec.protocol` | No researched library; Multi-protocol's [Other brokers (AMQP, cloud queues)](../architecture/07-multi-protocol.md#native-wire-proxy-stance) row; OQ-multi-protocol-14 option (a), adopted here |
...
| Amazon SNS | Yes | Yes | Not planned | No matching value in `Upstream.spec.protocol` | No researched library; Multi-protocol's [Other brokers (AMQP, cloud queues)](../architecture/07-multi-protocol.md#native-wire-proxy-stance) row; OQ-multi-protocol-14 option (a), adopted here |
...
| Feature | KrakenD CE | KrakenD EE | Ruralz status | Ruralz mechanism | Notes/ADR |
|---|---|---|---|---|---|
| Concurrent calls | Yes | Yes | Planned (M4) | Request hedging on the `Upstream` leg for idempotent, replayable requests | Partial parity: KrakenD sends N requests at once ([source](https://www.krakend.io/schema/v2.13/krakend.json)); a hedge waits a delay, needs an idempotent method and a replayable body, and spends retry budget ([Hedging](../architecture/09-traffic-management-and-resilience.md#hedging)). Hedging awaits `hedgeDelay` (proposed, OQ-traffic-management-and-resilience-5), which blocks M4 |
...
| Spike arrest and burst | Yes | Yes | Planned (M1) | `ratelimit` `limits[]` with a short `window` | `limits[].burst` ([Per-Node ceiling](../architecture/09-traffic-management-and-resilience.md#per-node-ceiling)), registered ([Registered from feature documents](../architecture/02-configuration-model.md#registered-from-feature-documents)) |
...
| Service rate limit | No | Yes | Planned (M1) | Gateway-scoped `ratelimit` with a constant `config.key` | Partial parity: a constant key is one State Store hot key: a limit above about 25,000 requests per second (hypothesis) needs a declared `limits[].perNodeCeiling` or `config.localOnly` ([Per-Node ceiling](../architecture/09-traffic-management-and-resilience.md#per-node-ceiling)), both registered ([Registered from feature documents](../architecture/02-configuration-model.md#registered-from-feature-documents)); `localOnly: true` still waits on the pack 8.8 amendment, OQ-scalability-and-distributed-state-11 (blocking M1) ([Distributed rate limits](../architecture/11-scalability-and-distributed-state.md#distributed-rate-limits)) ([source](https://www.krakend.io/docs/enterprise/service-settings/service-rate-limit/)) |
...
| Proxy rate limit | Yes | Yes | Planned (M1) | `ratelimit` with a constant key on each `Route` reaching the `Upstream`; `circuitBreaker.maxPendingRequests` | Partial parity: Upstream-scoped `ratelimit` per OQ-krakend-ee-parity-matrix-4. A constant key is one State Store hot key: a limit above about 25,000 requests per second (hypothesis) needs a declared `limits[].perNodeCeiling` or `config.localOnly`, both registered in the Configuration model; `localOnly: true` still waits on the pack 8.8 amendment, OQ-scalability-and-distributed-state-11 (blocking M1) |
...
| Fail-open distributed Rate Limits with bounded over-admission | Redis-backed limits are EE-only and block when Redis fails by default ([source](https://www.krakend.io/docs/enterprise/throttling/global-rate-limit/)) | `ratelimit` with a per-Node ceiling (ADR-0008): over-admission at most N × per-Node ceiling per window (target), with `limits[].perNodeCeiling` ([Per-Node ceiling](../architecture/09-traffic-management-and-resilience.md#per-node-ceiling)), registered in the Configuration model | Planned (M1) |
...
| AMQP/RabbitMQ consumer, AMQP/RabbitMQ producer, Azure Service Bus topic and subscription, Google Cloud Pub/Sub, Amazon SNS, Amazon SQS | Services connectivity | No | No researched Go library; six brokers and cloud queues under OQ-multi-protocol-14, whose current option (a) this matrix adopts | `kafka`, `nats` or `mqtt` Upstreams, or a bridge |
...
Seventeen planned rows are narrower than the KrakenD feature; each Notes/ADR cell starts with "Partial parity:" and says how. The transformation rows also depend on the `transform.*` `config` schema, authored in Data plane's [Transform Policies](../architecture/03-data-plane.md#transform-policies), pending Configuration model registration.
...
| Martian (DSL) | The DSL itself; its operations map to `headers` and `transform.*` | None |
...
| SOAP integration | Reading XML responses: CEL sees bodies only as JSON | OQ-data-plane-17 |
...
| Service rate limit | A constant key above about 25,000 requests per second (hypothesis) without extra declarations: it needs `limits[].perNodeCeiling`, or `config.localOnly` once pack 8.8 is amended | OQ-scalability-and-distributed-state-11 |
| Proxy rate limit | Upstream scope, and the same hot-key limit as Service rate limit | OQ-krakend-ee-parity-matrix-4; OQ-scalability-and-distributed-state-11 |
...
- **OQ-roadmap-and-milestones-1:** option (a) for every provisional allocation (the M1 transformation, access log and workflow rows and the M5 long-tail rows), except API governance, moved from M5 to M1 under option (b) because `quota` and `Consumer` `quotas` are Planned (M1). The Roadmap adopted the resulting exit figures (OQ-krakend-ee-parity-matrix-12, closed): M1 24 of 71, 34% (target); M2 36 of 71, 51% (target); M3 54 of 71, 76% (target); M4 56 of 71, 79% (target). M5 adds 12 rows instead of 13 and stays at 68 of 71, 96% (target).
...
- **OQ-multi-protocol-14:** this matrix adopts its current option (a), Not planned in M0 to M5, for all six rows the question names: AMQP consumer and producer, Azure Service Bus, Google Cloud Pub/Sub, Amazon SNS and Amazon SQS. Lambda stays under OQ-krakend-ee-parity-matrix-10.
- **Contingent M1 share:** five M1 EE-only rows, the `transform.*` rows, wait on Configuration model registration of the schema Data plane authored in [Transform Policies](../architecture/03-data-plane.md#transform-policies). Without it, M1 reaches 19 of 71, 27% (target), instead of 34% (target).
...
Closed: OQ-krakend-ee-parity-matrix-1 with option (a), decided by Data plane: [Transform Policies](../architecture/03-data-plane.md#transform-policies) is one CEL-based schema covering body extraction to headers, CEL-built bodies, CEL queries, regular expression replacement, flatmap array operations and XML request building; the SOAP XML response part moved to OQ-data-plane-17.

Closed: OQ-krakend-ee-parity-matrix-12 with option (a), adopted by the [Roadmap](../roadmap/01-roadmap-and-milestones.md#how-parity-is-counted): API governance at M1, cumulative parity of 24, 36, 54 and 56 of 71 at the M1 to M4 exits, and 12 EE-only rows at M5.
