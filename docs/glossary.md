...
| Upstream | A Bundle kind for a target service: Endpoints or discovery, protocol, load balancing, health checks, retries, circuit breaking, TLS and upstream-leg Policies. | Upstream (kind `Upstream`) | backend | [Configuration model: Upstream](architecture/02-configuration-model.md#upstream) |
