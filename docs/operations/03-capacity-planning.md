...
| Hot key | Above 25% of a shard: a declared per-Node ceiling, a split `config.key` or its own Cell; `localOnly` is proposed (OQ-traffic-management-and-resilience-1 (c), OQ-scalability-and-distributed-state-11) | λ_key × (1 + h) ≤ about 25,000; λ_slot × (1 + h) ≤ 50,000 per second (hypothesis) | [Rate limits](../architecture/11-scalability-and-distributed-state.md#distributed-rate-limits) |
...
- GCRA runs at up to 50% of q_shard (hypothesis), leaving margin under 70% script CPU for a Node count lagging 40% (hypothesis): 16 shards (target) carry 800,000 calls per second, 615,000 at peak with h (hypothesis), not [Cell sizing](../architecture/11-scalability-and-distributed-state.md#cell-sizing)'s 1,000,000 (62.5% of 16 shards).
