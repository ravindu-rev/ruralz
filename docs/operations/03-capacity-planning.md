...
This document turns workload figures into Node counts, container sizes, State Store shards and Ruralz Control replicas, with coefficients citing [Performance budgets and benchmarking](../architecture/12-performance-budgets-and-benchmarking.md), three worked examples, headroom, autoscaling bounds, costs and re-planning triggers. Nothing is implemented yet: every coefficient is a (target) or (hypothesis) until a published release record replaces it (P10). Its readers are operators sizing a Cell and architects checking Cell ceilings.

## Scope and non-goals

In scope: sizing Ruralz Gateway Nodes, each Cell's State Store, Ruralz Control replicas and the Control Store in one Region; Planned (M4) multi-region pre-provisioning applies it per Region. "Pack 8.x" cites the [foundation pack](../_meta/foundation-pack.md). It answers two questions it owns: OQ-deployment-topologies-8 with option (a), measured sizes replacing interim values once F2, the memory runs and Data plane's cache layout publish; and OQ-zero-downtime-upgrades-and-hot-reload-6 under [Headroom](#headroom-and-failure-capacity).
...
- Failover runbooks: [HA and DR](04-high-availability-and-disaster-recovery.md). Prices: operators, in the [Cost template](#cost-template).
...
| Mean duration with Upstream; session lifetime | W_r; L | `ruralz_http_request_duration_seconds` (unary); access-log `duration` |
...
| Routes; snapshot size | R; s_snap | `ruralz bundle build`; K-18 |
| L4 connection cap per Node; client reconnect window | C_cap; W_rc | Balancer setting; client jitter, a (hypothesis) for the client owner |
...
| Environments; Revision, diff and source sizes | E; s_rev, s_diff, s_src | Ruralz Control; s_diff ≈ 0.1 × s_rev, s_src ≈ s_rev (hypothesis), OQ-capacity-planning-3 |
| Revisions kept per Environment; older referenced ones | n_keep; n_ref | 50 (target), a [Control plane](../architecture/04-control-plane-and-gitops.md#building-and-recording-a-revision) policy; older referenced ones (active, Last-Known-Good, promoted), 3 until measured (hypothesis), OQ-capacity-planning-9 |
| Audit per day and days kept; Raft log; backups kept | d_audit; L_raft; n_bk | 10 MiB for 365 days, 1 GiB, 14 until measured (hypothesis); audit has no retention period (OQ-capacity-planning-9) |
...
The model sizes each Cell independently: a Cluster's Nodes use only their Cell's State Store ([Cells](../architecture/11-scalability-and-distributed-state.md#cells-and-blast-radius)) and no request crosses a Region to reach one (pack 8.13). Node count follows whichever binds first: CPU, connections, in-flight units or buffered bytes; memory follows the flood case.
...
Every coefficient cites [Performance budgets and benchmarking](../architecture/12-performance-budgets-and-benchmarking.md) (PB) or, where PB has no value yet, its owner and the Open question that adds it to PB. Configuration defaults cite their owner; retention counts are policies in [Planning inputs](#planning-inputs). PB adopts the latest published release record ([Production comparison](../architecture/12-performance-budgets-and-benchmarking.md#production-comparison)). CPU values are per 4-vCPU RH-1 core set; other hardware needs its own ladder run.
...
| K-4 | c_est, per prompt KiB estimated; c_hs, per TLS 1.3 handshake | 25 µs; 500 µs (hypothesis) | Per-stage budget; Resources per core | PB |
...
| K-11 | u_s, units per SSE response; per WebSocket session | 1; 3 (hypothesis) | [Milestone plan](../architecture/12-performance-budgets-and-benchmarking.md#milestone-plan), M3 session memory; units pending (OQ-capacity-planning-4) | Data plane; OQ-capacity-planning-1 |
...
| K-14 | m_key, bytes per limit or Quota key; calls per Node for an unknown mix | 128 B; 5,000 per second (hypothesis) | Pending; G1 counts calls, not bytes (OQ-capacity-planning-2) | This document; [Sizing defaults](01-deployment-topologies.md#sizing-defaults) |
...
| K-16 | Heartbeat interval | 15 s (target) | None (configuration default) | Control plane |
| K-17 | Largest batch at 10,000 Nodes | 5 minutes (target) | Control-plane timing | Control plane |
| K-18 | K retired snapshots; grace; snapshot heap per Route | 2; 30 s; 0.5 KB (target) | Constants; [Reload](../architecture/12-performance-budgets-and-benchmarking.md#startup-and-reload-time-by-configuration-size) | Data plane |
| K-19 | s_rev per resource; replica container; Enrollments per second per leader | About 1.5 KiB; 2 vCPU, 4 GiB; about 20 (hypothesis) | Pending (OQ-capacity-planning-3) | This document; Sizing defaults; [RA-3](01-deployment-topologies.md#ra-3-multi-region-active-active-with-cells) |
| K-20 | e, prompt estimate error (SM-10) | 2% (hypothesis) | [Benchmark suite](../architecture/12-performance-budgets-and-benchmarking.md#benchmark-suite-and-ci-gating), SM-10 | [Vision](../vision/01-vision-and-positioning.md#success-metrics); OQ-vision-and-positioning-15 |
| K-21 | P_plg, `limits.maxPluginMemoryBytes` default | 2 GiB (target) | Memory budget | WASM plugin system |
| K-22 | `GOMEMLIMIT` share of container; `maxmemory` use; shard GCRA share | 0.9; 0.7; 0.5 (target) | [GOGC and GOMEMLIMIT](../architecture/12-performance-budgets-and-benchmarking.md#gogc-and-gomemlimit) rule 1 for 0.9; Throughput targets, G1, for 0.5; 0.7 and 0.5 pending (OQ-capacity-planning-2) | [Scale unit](../architecture/11-scalability-and-distributed-state.md#scale-unit); Cell sizing |
...
Memory per Node
  ...
  P_plg  = K-21 when a Route attaches a `plugin` Policy, else 0
  M_node  = M_fix + m_route × R + P_plg + (1 + h) × (C × m_conn + I × m_unit + C_up × m_up) / N_s
            conservative, as M_fix reserves the full maxBufferedBytes and state tables
...
State Store per Cell
  ...
  n_shard = max(ceil((1 + h) × O / (0.5 × q_shard)), ceil(N_max × λ_node × s / (0.5 × q_shard)))  (hypothesis)
            at most 16 shards (target), else split the Cell
  λ_node  = one Node's steady rate at its scale-out threshold, per workload class:
            request-bound: V × u_z / c_r requests per second                          (target)
            connection- or stream-bound: threshold sessions / L opens per second,
            f_z × I_max / (u_s × L) when units bind                                   (target)
            burst upgrades and opens are checked only by the W_rc,min rule below
...
Ruralz Control deployment
  ...
  D_store  = E × (n_keep + n_ref) × (s_rev + s_diff + s_src) + d_audit × days kept
             + L_raft + two Raft snapshots
  audit growth = d_audit per day while audit has no retention period (OQ-capacity-planning-9);
             days to 70% = (0.7 × volume − (D_store − d_audit × days kept)) / d_audit
  volume   ≥ D_store / 0.7                                                              (target)
```

Every `https` listener negotiates HTTP/2 by ALPN and every `http` listener accepts h2c ([Listeners](../architecture/03-data-plane.md#listeners-and-protocols)), so 80 connections can hold all 20,000 units (target): an L4 connection cap alone does not bound memory, and M_flood charges I_max × m_u, 6,250 MiB (hypothesis), on every Node until OQ-capacity-planning-5 closes. `ruralz_http_active_requests` counts requests and sessions, not parallel steps or pumps, undercounting units by (1 + q_r) on composition Routes and by u_s on streams.

## Worked examples

Every figure below is a (target) for a three-zone Cell (Z = 3, u_z = 0.53, f_z = 0.5), h = 30% (target). [H] marks lines resting on a (hypothesis) coefficient, re-derived when its benchmark publishes.
...
m_stream = 96 KiB client + 96 KiB provider TLS + 32 KiB share + 80 KiB prompt = 304 KiB  (target) [H]
...
λ_node   = f_z × I_max / (u_s × L) = 10,000 / (3 × 3,600 s) = 0.93 opens per second  (target) [H]
           rule 2: floor(1 × 50,000 / (0.93 × 1)) = 53,763, above 1,000 per Cell and 104  (target) [H]
           n_shard second term: ceil(104 × 0.93 × 1 / 50,000) = 1 shard                (target) [H]
...
When a third snapshot would be retired, streams pinned to the oldest end at once with 1001 ([Data plane](../architecture/03-data-plane.md#why-no-in-flight-request-is-dropped)); only client jitter spreads them, not the 30 s grace (target) (OQ-capacity-planning-7). Clients faster than W_rc,min need `strategy: canary`, shards or CPU. Activations 90 minutes apart, 1.5 × L (target), keep each retirement near 5% of sessions (hypothesis); the 4 h `timeout` stops hourly resynchronized expiry.

## Headroom and failure capacity

Capacity covers peak plus h on the survivors of the largest single failure. Failures compose only where the table says; every example checks the composite zone loss.

| Failure or event | Capacity rule | Formula or value | Owner of the bound |
|---|---|---|---|
| Node loss | At least three Nodes; clients retry | N ≥ 3 (target) | [Gateway availability](../architecture/11-scalability-and-distributed-state.md#ruralz-gateway-availability) |
...
| Hot key | Above 25% of a shard: a declared per-Node ceiling, a split `config.key` or its own Cell; `config.localOnly` is proposed (OQ-scalability-and-distributed-state-11) | λ_key × (1 + h) ≤ about 25,000; λ_slot × (1 + h) ≤ 50,000 per second (hypothesis) | [Rate limits](../architecture/11-scalability-and-distributed-state.md#distributed-rate-limits) |
...
| Hot Reload under load | Configuration memory and reload time | (K + 2) × snapshot; 1 s at 5,000 Routes (target) | [Reload time](../architecture/12-performance-budgets-and-benchmarking.md#startup-and-reload-time-by-configuration-size) |
| Zero-Downtime Upgrade on a VM | Answers OQ-zero-downtime-upgrades-and-hot-reload-6 with option (a) for planning: two processes, each `GOMEMLIMIT` at M_flood ([ADR-0015](../adr/0015-zero-downtime-upgrades-so-reuseport.md)) | Host memory ≥ 2 × container (hypothesis): 26 GiB for Example 1 (target); (b) below | [Zero-downtime upgrades](02-zero-downtime-upgrades-and-hot-reload.md) |
| Ruralz Control replica or zone loss | Survivors at 80% of shedding onset | r_min (hypothesis) | [Scaling](../architecture/11-scalability-and-distributed-state.md#scaling-ruralz-control-and-the-state-store) |

Under option (b), once its shared ceilings are verified, the new process runs under 90% of the limit minus the old RSS (target), admitting each ceiling minus the old process's usage. Independent limits below M_flood are not allowed: halving memory does not merely halve C_cap, as the 6,250 MiB (hypothesis) unit term is fixed, so a mid-Drain flood would OOM-kill a process.

```text
VM host memory during a handover (OQ-zero-downtime-upgrades-and-hot-reload-6)
  (a) planning:  host ≥ 2 × container                                            (hypothesis)
                 Example 1: 2 × 13 GiB = 26 GiB                                   (target) [H]
  (b) verified shared ceilings: host ≥ container + overlap extra, about 0.6 GiB
                 plus (K + 2) × s_snap                                            (hypothesis)
                 Example 1: 13 GiB + 0.6 GiB + 1 MiB ≈ 13.6 GiB                   (target) [H]
  independent split: 0.45 × host ≥ M_flood at C_cap', so
                 C_cap' = (0.45 × host − M_flood without the C_cap × m_c term) / m_c  (hypothesis)
                 Example 1 floor without connections: 1,245 + 500 + 6,250 + 469
                 = 8,464 MiB > 0.45 × 13,312 = 5,990 MiB even at C_cap' = 0         (target) [H]
                 Example 3: 1,243 + 500 + 6,250 + 146 = 8,139 MiB > 5,990 MiB      (target) [H]
                 smallest Example 1 split host: 2 × 11,282 / 0.9 = 24.5 GiB, so
                 no split below the 26 GiB of (a)                                 (target) [H]
```

At 80% of saturation, S2 allows a 2 ms gateway-added p99 (target, PB-16), so a zone loss burns SLO-GW-2 until OQ-performance-budgets-and-benchmarking-13 decides; a Cluster that must hold 1 ms (target) plans with u_z = 0.6 × (Z − 1) / Z (target).

## Autoscaling policies

Autoscaling reacts inside the plan, never replacing it. The HorizontalPodAutoscaler is Planned (M2) ([T4](01-deployment-topologies.md#t4-kubernetes-with-helm-hpa-and-crds)); VM groups follow the same rules. Thresholds use f_z and scale request gauges by units per request.
...
| Connection-bound, Example 3 | In-flight units | `ruralz_listener_open_connections` or `ruralz_http_active_requests` at f_z × 20,000 / u_s = 3,333 (target) | N, held static, as connections pin to Nodes | N plus one zone's share, 104 (target) |
...
2. maxReplicas MUST NOT exceed floor(n_shard × 0.5 × q_shard / (λ_node × s)) (hypothesis), with λ_node per workload class ([Formulas](#formulas)), or 1,000 per Cell (target), since autoscaling misses State Store saturation: 13 for Example 1, rounded to 12; 53,763 for Example 3 (target) [H].
...
4. In Control mode the published Node count rises at most 10% per minute after a 10-minute qualification (target) ([Replica roles](../architecture/04-control-plane-and-gitops.md#replica-roles)), so autoscaled Clusters SHOULD declare per-Node ceilings (pack 8.8, [ADR-0008](../adr/0008-rate-limiting-local-bucket-and-gcra.md); field `limits[].perNodeCeiling`).
5. Enrollment MUST keep up with the largest scale-out step per egress address (K-19).
6. Ruralz Control replicas and State Store shards are never autoscaled: adding a shard moves keys and a voter changes quorum ([ADR-0006](../adr/0006-control-store-raft-boltdb.md)).

## State Store sizing

The State Store is a Cell's only shared request-path dependency, so shards are sized from operations. Rate Limits pair a local token bucket with GCRA in one Lua `EVAL` ([ADR-0008](../adr/0008-rate-limiting-local-bucket-and-gcra.md)); local denials and over-limit cache hits cost no call.
...
Consumptive calls whose keys share a hash tag run as one script (pack 8.7), so s can be below the Policy count; for an unknown mix, plan K-14.
...
- GCRA runs at up to 50% of q_shard (hypothesis), leaving margin under 70% script CPU for a Node count lagging 40% (hypothesis): 16 shards (target) carry 800,000 calls per second, 615,000 at peak with h (hypothesis), matching [Cell sizing](../architecture/11-scalability-and-distributed-state.md#cell-sizing).
- A hot key cannot be split and hash tags gather a Consumer's keys, so λ_key and λ_slot are checked apart ([Hot key](#headroom-and-failure-capacity)).
...
- Two of three zones reach a zonal primary at a 2 ms p99 (hypothesis), PB-14; `stateStoreTimeout` stays at 5 to 10 ms (target).
...
- A layout change adds no calls (pack 8.7). Only Quota counters on the switch-boundary path may admit up to twice the Consumer quota for one window per Node lacking the signal (hypothesis); GCRA and Token Budget counters admit nothing extra ([Layout changes](../engineering/04-release-versioning-and-compatibility.md#state-store-layout-changes)).
- Limits are exact only inside one Cell ([bounds](../architecture/11-scalability-and-distributed-state.md#consistency-and-accuracy-bounds)), so a Consumer reaching C_cells Cells gets up to C_cells × each limit (target), so a split pins each Consumer to one Cell by hostname or tenant, or divides limits with one Environment per Cell ([Divided](../architecture/11-scalability-and-distributed-state.md#regional-versus-global-quotas)).

## Ruralz Control sizing

Ruralz Control, Planned (M2), is off the request path, so its size follows Node count, Revision size and retention ([Formulas](#formulas)). Relay-served Nodes, Planned (M4), count toward N_all as in Sizing defaults, safely overstating replica streams.
...
| Control Store content, n_keep = 50 (target), n_ref = 3 | 159 Revisions × 17.6 MiB ≈ 2.7 GiB (target) [H] |
| Audit, Raft log, two Raft snapshots | 3.6 + 1 + 0.2 GiB (target) [H] |
| Volume per replica | D_store 7.5 GiB / 0.7, so 20 GiB (target) [H] |
| Audit growth, no retention period | 10 MiB per day: 70% of the volume after about 1,030 days (target) [H] |
...
Every replica becomes a voter once it catches up after `ruralz control join` ([Control plane](../architecture/04-control-plane-and-gitops.md#certificates-and-replica-join)); a deployment serves at most 10,000 Nodes (target). In three zones, three voters keep zone-loss survivors at or below 4,000 streams, 80% of the shedding onset, up to 8,000 Nodes (hypothesis); five voters (target) keep 3,333 at 10,000 Nodes and quorum when the zone holding two fails, stricter than Scalability's ceil(N / 5,000) + 1, which leaves survivors at the onset. Each `ruralz control backup` of the Control Store ([ADR-0006](../adr/0006-control-store-raft-boltdb.md), proposed) is about D_store; [HA and DR](04-high-availability-and-disaster-recovery.md) owns restores.

## Cost template

Monthly cost is Σ quantity × operator-supplied unit price × hours.
...
| Ruralz Control replicas | replicas × (K-19 container, 20 GiB volume) | 3 (target) | 3 (target) | 3 (target) |
| L4 load balancer | Connections; new per second | 12,000; 200 (target) | 500; 25 (target) | 200,000; 56, 6,667 in a storm (target) [H] |
| Cross-zone traffic | Two thirds of State Store calls and Upstream legs or messages, without locality | 13,333 calls, 40,000 legs per second (target) | 33 calls per second (target) | 37 calls, 26,667 messages per second (target) |
| Access-log records | λ_r, `accessLog.when` unset | 20,000 per second (target) | 25 per second (target) | 56 per second (target) |
| Backups | n_bk × D_store | 105 GiB, shared (target) [H] | Shared | Shared |

Ruralz Control's cost divides across its Cells. No cost depends on a license or edition: every build has every feature (P1).

## Re-planning triggers

Each trigger is measured or a checkable event, and reruns the model for its Cell.
...
| Ceilings | `ruralz_listener_open_connections`; `ruralz_http_active_requests` × units per request; `ruralz_node_buffered_bytes` | Above the autoscaling thresholds at peak (target) | Recompute N_conn, N_unit, N_buf |
...
| Control Store volume | Volume use per replica | Above 70% (target) | Check audit retention (OQ-capacity-planning-9) or resize |
...
|---|---|---|---|---|
| OQ-capacity-planning-1 | Does an upgraded WebSocket session hold three in-flight units (upgrade plus two pumps) or two? | (a) Three (current); (b) two, lowering Example 3 to 54 Nodes (target) | data-plane | Yes, for WebSocket sizing (M3) |
| OQ-capacity-planning-2 | Which bytes per key do limit, Quota and cache keys use, which per-Node call rate replaces K-14, and do K-22's 0.7 and 0.5 shares hold? | (a) G1 and S5 variants measure them (proposed); (b) keep K-14 | performance-budgets-and-benchmarking | No |
| OQ-capacity-planning-3 | What are s_rev per resource, a replica's container and memory per Control Stream, and Enrollments per second per leader? | (a) F2 and R1 record them (proposed); (b) keep K-19 | performance-budgets-and-benchmarking | No |
| OQ-capacity-planning-4 | Should PB verify stream-share admission (K-10) and the leg and merge costs K-2, and record units per session (K-11) in its M3 session-memory run? | (a) A composition scenario and a stream-share constant (proposed); (b) no | performance-budgets-and-benchmarking | No |
| OQ-capacity-planning-5 | Should small or connection-heavy Nodes get a lower in-flight unit or HTTP/2 stream ceiling, since an L4 cap leaves 6,250 MiB (hypothesis) of unit memory in every flood floor and blocks VM handover memory splits? | (a) Fixed 20,000 units and 250 streams (current); (b) Gateway `limits` fields, with OQ-deployment-topologies-21 and OQ-data-plane-1 | data-plane | No |
...
| OQ-capacity-planning-7 | Should closing snapshots jitter stream closes across the grace period, as Drain does, and long-lived streams jitter their `timeout` end (OQ-scalability-and-distributed-state-6)? | (a) No, relying on client jitter W_rc (current); (b) jitter both over 30 s (target) | data-plane | No |
...
| OQ-capacity-planning-9 | Which retention period applies to audit segments, and how many Revisions older than the latest 50 per Environment stay referenced (n_ref)? | (a) A fixed audit period and a measured n_ref (proposed); (b) pruning audit after `ruralz control backup`; (c) none | control-plane-and-gitops | Yes, for Control Store sizing (M2) |
