...
| gRPC upstream client | grpc-go v1.84.x; in M1 only the transport of the OTLP/gRPC exporters (they require v1.83.1) | connect client | Native HTTP/2 transport and full client semantics | Apache-2.0 | OTel metric names changed in v1.84.0; depguard confines it to `internal/telemetry` until M3 | Verified | None | Planned (M1) OTLP export; Planned (M3) gRPC Upstreams | ([source](https://github.com/grpc/grpc-go/releases/tag/v1.84.0)) ([source](https://github.com/grpc/grpc-go)) ([source](https://proxy.golang.org/go.opentelemetry.io/otel/exporters/otlp/otlptrace/otlptracegrpc/@v/v1.46.0.mod)) |
...
| Telemetry | `go.opentelemetry.io/otel` v1.46.x traces and metrics over OTLP/gRPC; `log/slog` with `otelslog` v0.20.x; `go.opentelemetry.io/proto/otlp` v1.11.x comes with the exporters, imported directly only by the test collector | Vendor SDKs; Prometheus client only | Traces and metrics stable; Logs API expected at v1 in v1.47.0 | Apache-2.0 | v1.47.0-rc.1 deprecates `otel/log/global`; `/metrics` is the Metrics exposition row | Pending CI | [ADR-0010](../adr/0010-telemetry-opentelemetry-first.md) | Planned (M1) | ([source](https://github.com/open-telemetry/opentelemetry-go/blob/main/README.md)) ([source](https://github.com/open-telemetry/opentelemetry-go/releases/tag/v1.47.0-rc.1)) ([source](https://proxy.golang.org/go.opentelemetry.io/otel/exporters/otlp/otlptrace/otlptracegrpc/@v/v1.46.0.mod)) ([source](https://proxy.golang.org/go.opentelemetry.io/proto/otlp/@v/v1.11.0.zip)) |
| Metrics exposition (`/metrics`) | `go.opentelemetry.io/otel/exporters/prometheus` v0.68.x on a private `client_golang` v1.24.x registry, with `prometheus/common` v0.71.x (`expfmt`) and `prometheus/otlptranslator` v1.0.x (`UnderscoreEscapingWithoutSuffixes`), in `internal/telemetry` only (OQ-tech-stack-and-libraries-16) | Prometheus client alone | One instrument set feeds OTLP and `/metrics`, so both expose one name set; `WithProducer` reads Ruralz-owned aggregates | Apache-2.0 | Pre-1.0; the default strategy appends suffixes, so it MUST be set; otlptranslator's newest release is from 2025-09-09 (fails S1) | Pending CI | [ADR-0010](../adr/0010-telemetry-opentelemetry-first.md) | Planned (M1) | ([source](https://proxy.golang.org/go.opentelemetry.io/otel/exporters/prometheus/@v/v0.68.0.mod)) ([source](https://proxy.golang.org/go.opentelemetry.io/otel/exporters/prometheus/@v/v0.68.0.zip)) ([source](https://github.com/prometheus/client_golang/releases/tag/v1.24.1)) ([source](https://proxy.golang.org/github.com/prometheus/common/@v/v0.71.0.zip)) ([source](https://proxy.golang.org/github.com/prometheus/otlptranslator/@v/list)) |
...
| JOSE | `lestrrat-go/jwx/v4` v4.5.x; `ruralzd` uses jwx alone for JWKS and token verification; `jwk`, `jws` and `jwa` link `lestrrat-go/dsig` v1.4.x, `lestrrat-go/option/v3` v3.0.0-alpha1 and `valyala/fastjson` v1.6.x, not `golang.org/x/crypto` | golang-jwt/jwt/v5; go-jose/v4 alone | Full JWA, JWE, JWK, JWS, JWT; v4.5.0 fixes GHSA-4cf7-xm37-g63h | MIT (jwx and the three linked modules) | `GOEXPERIMENT=jsonv2` on Go 1.26 (S2 exception); ML-DSA unregistered below Go 1.27 (G3); `option/v3` is a pre-release | Pending CI | None | Planned (M1) | ([source](https://github.com/lestrrat-go/jwx/blob/develop/v4/README.md)) ([source](https://github.com/lestrrat-go/jwx/releases/tag/v4.5.0)) ([source](https://proxy.golang.org/github.com/lestrrat-go/jwx/v4/@v/v4.5.0.mod)) ([source](https://proxy.golang.org/github.com/lestrrat-go/dsig/@v/v1.4.0.zip)) ([source](https://proxy.golang.org/github.com/lestrrat-go/option/v3/@v/v3.0.0-alpha1.zip)) ([source](https://proxy.golang.org/github.com/valyala/fastjson/@v/v1.6.10.zip)) |
...
| Expressions | cel-go v0.32.x at `cel.dev/cel-go` (`google/cel-go` is a read-only alias), with `cel.dev/expr` v0.25.x, whose types cel-go's API returns; both are confined to `internal/cel` | Lua; expr-lang | Typed, non-Turing-complete; `CostLimit`, `ContextEval` | Apache-2.0 | Old path "will eventually be removed" | Pending CI | [ADR-0011](../adr/0011-expressions-and-authorization-engines.md) | Planned (M1) | ([source](https://github.com/google/cel-go/blob/main/README.md)) ([source](https://github.com/cel-expr/cel-go/releases/tag/v0.32.0)) ([source](https://proxy.golang.org/cel.dev/cel-go/@v/v0.32.0.mod)) ([source](https://proxy.golang.org/cel.dev/expr/@v/v0.25.2.zip)) |
...
| Zero-Downtime Upgrade socket steering, peer credentials and test namespaces | `golang.org/x/sys/unix` v0.48.x, imported directly for `SO_ATTACH_REUSEPORT_CBPF` steering, `SO_PEERCRED` on the handover socket and, in tests only, network namespaces for `internal/testkit/proc` (`CLONE_NEWNET`, loopback brought up through the `SIOCSIFFLAGS` ioctl, per-namespace sysctls) | `syscall` | Already linked through grpc-go in M1 and through wazero and bbolt from M2, so no new module | BSD-3-Clause | Steering, peer credentials and namespaces are Linux-only | Pending CI | [ADR-0015](../adr/0015-zero-downtime-upgrades-so-reuseport.md) | Planned (M1) | ([source](https://github.com/golang/sys/blob/v0.48.0/LICENSE)) ([source](https://proxy.golang.org/golang.org/x/sys/@v/v0.48.0.zip)) ([source](https://proxy.golang.org/google.golang.org/grpc/@v/v1.84.0.mod)) ([source](https://github.com/wazero/wazero/blob/main/go.mod)) ([source](https://github.com/etcd-io/bbolt/blob/v1.4.1/go.mod)) |
| ULID for `node.id` | Own code on `crypto/rand` in `internal/ulid` (OQ-tech-stack-and-libraries-15) | `oklog/ulid/v2` v2.1.x | One identifier per Node needs no module (S6) | Apache-2.0 (Ruralz code) | Encoding is Ruralz code and tests | Ruralz code | None | Planned (M1) | ([source](https://pkg.go.dev/crypto/rand)) ([source](https://github.com/oklog/ulid/releases/tag/v2.1.2)) |
| CLI framework | Standard library `flag`; interspersed flags, help and completion are Ruralz code in `internal/cli/command` (OQ-tech-stack-and-libraries-18) | `spf13/cobra` v1.10.x | No dependency; commands are an explicit table | BSD-3-Clause (standard library) | `flag` stops at the first positional argument | Standard library | None | Planned (M1) | ([source](https://pkg.go.dev/flag)) ([source](https://github.com/spf13/cobra/releases/tag/v1.10.2)) |
...
| OQ-tech-stack-and-libraries-15 | ULID library? | (a) oklog/ulid/v2; (b) Chosen: own code on `crypto/rand` in `internal/ulid` ([Library catalog](#library-catalog)) | Data plane owner | No (answered) |
| OQ-tech-stack-and-libraries-16 | Which `/metrics` exporter serves 9901 and 9902? | (a) Chosen: OTel Prometheus exporter on a private client_golang registry, in `internal/telemetry` for both admin ports ([Library catalog](#library-catalog)); (b) Prometheus client | Observability owner | No (answered) |
| OQ-tech-stack-and-libraries-17 | SigV4 signer? | aws-sdk-go-v2 signer/v4; own code | Security and identity owner | No |
| OQ-tech-stack-and-libraries-18 | CLI framework? | (a) spf13/cobra; (b) Chosen: standard library `flag` with Ruralz interspersed parsing ([Library catalog](#library-catalog)) | CLI reference owner | No (answered) |
