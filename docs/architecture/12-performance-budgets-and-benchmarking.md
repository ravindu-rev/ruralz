...
- CI stages and jobs: [Testing and quality strategy](../engineering/03-testing-and-quality-strategy.md#benchmarks-and-regression-gates); this document places gates inside them.
...
| `RZ-RT-005` rejection (O1) | Header decode, admission check, 503 write | 16 (target) | 40 (target) | 2 (target) |
...
2. S1's stages sum to 45 µs at p50 and 177 µs at p99 (target), leaving PB-1 823 µs (target) of reserve.
3. The TLS 1.3 handshake costs 1 ms or less of CPU at p99 with ECDSA P-256 (target), measured in S3 and C2.
4. Restated: CEL expressions under 2 µs at p99 (target) ([Configuration model](02-configuration-model.md#limits)); `authz.opa` and `authz.cedar` under 100 µs (hypothesis), Planned (M2) ([Security and identity](08-security-and-identity.md)).

RS256 assumes about 40 µs of CPU (hypothesis), measured in M1 (algorithm: OQ-performance-budgets-and-benchmarking-8); a 16 KiB prompt's estimate, about 400 µs (hypothesis), is why PB-12 excludes it.
...
| RH-3 | Reproduction profile | A cloud VM with 4 dedicated vCPUs | Published, never gated |

Results record the CPU microarchitecture and never compare across generations: Green Tea GC gains about 10% more on Ice Lake and Zen 4 or newer ([source](https://go.dev/doc/go1.26)). With SMT off, "per core" and "per vCPU" name one figure. The seeds measure 4 vCPU together: 10,000 rps per core means 40,000 rps on the cpuset (target), not a single-core Node, where queueing at 80% utilization alone would push p99 past 1 ms (hypothesis).
...
| S3 HTTP/2 over TLS 1.3 | 40,000 rps or more (target) | 10,000 rps or more (target) | 1 ms p99 at half saturation (target) | Planned (M1); HTTP/3 variant Planned (M3) |
...
S6: 1,000 streams × 100 chunks/s × 12 µs = 1.2 vCPU (target); an onChunk Plugin adds 20 µs per chunk
```
...
|---|---|---|
| Idle RSS, no Revision, telemetry at defaults | 92 MiB or less (target) | Replaces [Tech stack](../engineering/01-tech-stack-and-libraries.md#library-catalog)'s 96 MiB (OQ-performance-budgets-and-benchmarking-5) |
| Base RSS with R Routes (PB-6) | 100 MB + 2 KB × R or less (target) | Seed; 100 to 17,000 Routes |
| Sharded label sets, first 1,000 admitted Routes | 2 MiB of RSS, 1 MiB live, 4 stripes (target), in the base | About 1 KiB per Route (hypothesis), [Observability](10-observability.md#overhead-per-signal), OQ-performance-budgets-and-benchmarking-10 |
...
PB-6 closes at every checked size, with about 1.4 MB of margin at 1,000 Routes (target):

```text
PB-6 allowance      100,000,000 + 2,000 × R bytes                              (target)
Idle base           92 MiB = 96,468,992 bytes                                  (target)
Sharded label sets  2 MiB = 2,097,152 bytes, all of it from R = 1,000          (target)
RSS slope           2 × (500 snapshot + 500 label sets) = 2,000 bytes per Route (target)
R = 0               96,468,992                                        <= 100,000,000
R = 1,000           96,468,992 + 2,097,152 +  2,000,000 = 100,566,144 <= 102,000,000
R = 17,000          96,468,992 + 2,097,152 + 34,000,000 = 132,566,144 <= 134,000,000
```
...
**OQ-observability-17.** This document chooses option (a) with one change: idle RSS drops from 96 to 92 MiB (target) to fit the 100 MB seed with the sharded label sets (OQ-performance-budgets-and-benchmarking-5); the loaded-RSS row checks that the seeds fit together. At 40,000 rps on 4 vCPU (target), a Node spends at most 92 MiB idle, 80 MiB on telemetry, about 6 MiB each on 256 client and 256 upstream connections, and 72 MiB on in-flight buffers and GC headroom, within 256 MiB (target). Observability's 50,000 rps per Node (hypothesis) matches PB-4. A documented `GOMEMLIMIT`, option (d), stays deployment guidance ([Scalability](11-scalability-and-distributed-state.md#scale-unit)).
...
The stages match the `stage` label of `ruralz_config_activation_duration_seconds` and the steps of [Compile before swap](01-system-overview.md#compile-before-swap). For 5,000 Routes, idle RH-1 Node:
...
The loader's W = max(1, `GOMAXPROCS`/2) workers yield at least every 100 µs of work (target), so requests never wait behind a compile slice (OQ-performance-budgets-and-benchmarking-6). Under S1 at half saturation, a 5,000-Route reload takes 1 s or less (target), and gateway-added p99 stays at 1.5 ms or less (hypothesis). A 2 MiB Plugin's cold compile takes up to 1 s on one core (hypothesis) ([WASM plugin system](05-wasm-plugin-system.md#performance-model)), so `plugin_compile` is outside PB-7 and SLO-GW-6.
...
| S5x | As S5, 1 ms added each way on RH-1S (target), tool per OQ-testing-and-quality-strategy-3 | As S5 | HTTP/1.1 | Planned (M1) |
| S6 | Mock `openai` provider, 100 chunks per second on 1,000 streams, 16 KiB prompts (target) | `ai.token-budget`, `memory` driver | SSE | Planned (M3) |
...
|---|---|---|---|
| O1 | S3's chain on 120 connections of 250 streams (target). O1a: mock delay 1 s, 30,000 rps, 150% of what the in-flight ceiling admits. O1b: no delay, 60,000 rps, 150% of S3 saturation | Admitted plus `RZ-RT-005` within 0.5% of offered; active requests plateau at 20,000; O1a rejects at p99 of 1 ms or less; O1b meets PB-11 (target) | Planned (M1) |
...
| F1 | SM-9: 100 `ruralzd` processes, three Ruralz Control replicas | PB-13 | Planned (M2) |
| F2 | A Control Stream driver emulating 10,000 Nodes (mTLS, 15 s heartbeats (target), ACK after simulated activation) against three replicas | [Fleet-level timing](#fleet-level-timing), from `ruralz_control_connected_nodes` and `ruralz_control_stream_reconnects_total` | Planned (M2) |

```text
O1b: 20,000 rejections/s × 16 µs = 0.32 vCPU, 8% of 4 vCPU → goodput 92% of saturation   (target)
O1a: 20,000 rps admitted + 10,000 rejected/s stays under 2.2 vCPU → rejections never queue (target)
O1b queueing: the fixed ceiling admits 20,000 units, so admitted requests wait about
      20,000 / 36,800 rps ≈ 0.54 s until an adaptive limiter lands (OQ-data-plane-6)  (hypothesis)
...
O2:  8,192 calls / 0.5 s = 16,384 calls/s held; the other 7,616/s are skipped           (target)
      (CE-3's 200 ms would need 41,000 calls/s to fill 8,192; the 1 s timeout keeps the breaker closed)
```
...
spec:
  ...
  telemetry:
    ...
    traceSampling: 0.01              # set explicitly; accessLog.when unset: every request is logged
  stateStore:
...
metadata:
  name: jwt-bench
spec:
  type: auth.jwt                     # generator rotates 10,000 tokens from two keys; no verified-token cache
  config:
    issuers:
...
spec:
  type: ratelimit
  config:
    key: 'request.headers["x-bench-key"]'   # the generator cycles 100,000 values uniformly
    limits: [{requests: 20000, window: 1s}] # under 25,000/s per key (target); never denies at the offered rate
# At 40,000 rps each key sees about 0.4 requests/s, so S5 runs GCRA on every request and denies none.
# S5 and O2 overlays set stateStore {driver: redis, url: secretRef env RURALZ_STATE_STORE_URL};
# O2's overlay also sets the Policy's stateStoreTimeout: 1s.
# A Zipf variant keeps its hottest key under 25,000 calls/s; single-key runs belong to CE-12.
```
...
- **Warm-up.** Each run first offers its rate for 60 s (target), discarded, extended in 30 s steps up to 180 s until the p99 of three consecutive 10 s windows varies by 5% or less (target).
...
- **Rate ladder.** Open-loop steps of 5% of the expected saturation, 60 s each (target).
...
- Each result records the hardware fingerprint, kernel, sysctls, NIC settings, cpuset, `GOMAXPROCS`, `GOGC`, `GOMEMLIMIT`, the toolchain (go1.27.1, pack 7; [ADR-0001](../adr/0001-implementation-language-go.md)), commit, Revision digest, tool versions and command lines.
...
| Component benchmarks | Loader compile ladder, JWT per algorithm, token bucket shards, `RZ-RT-005` rejection, metric aggregates at 4 P and 32 P; tokenizer, Planned (M3) | RH-1, `nightly` Latency job | Planned (M1) |
| Scenarios | S1 to S8, S5x, S6n, O1, O2, C1, C2, R1, G1, constants tests | RH-1 hosts, Latency job: S1 and S2 nightly, the rest in rotation | Planned (M1) to Planned (M4) |
| Fleet and accounting | F1 (SM-9), F2, SM-10 | Dedicated general runners, `nightly` Scale job (F2: OQ-performance-budgets-and-benchmarking-3) | Planned (M2) to Planned (M3) |
| Soak | S2 at half saturation for 2 hours: RSS drift of 2% or less after 10 minutes, flat goroutines (target) | RH-1, `release` | Planned (M1) |
...
```text
S1 and S2 A/B: 2 × (10 runs × 180 s + 10 warm-ups × 60 s)   80 min (target)
Component benchmarks                                          15 min (target)
One rotation slot per night                                   75 min (target)
Total                                                        170 min (target)
Rotation, so each item runs at least weekly (target):
  S3 and S5 | S5x and G1 | O1 and O2 | C1, C2 and R1 | saturation ladders
  with the PB-15 and PB-16 points | S4 (M2) | S6, S6n and S7 (M3)
The 2-hour soak exceeds a slot, so it runs in release only.
```
...
| Size and idle RSS | `pr-full` | Fixed values | Stripped binary above 160 MiB or idle RSS above 92 MiB (target) | Planned (M1) |
| Fleet and accounting | Scale job; `release` | PB-13, the F2 rows, SM-10 | Any missed | Planned (M2) to Planned (M3) |
...
For OQ-testing-and-quality-strategy-2 this document chooses option (c): alloc/op on shared runners, because allocation counts do not depend on the machine under `GOGC=off` with fixed `GOMAXPROCS`, and latency only on RH-1 bare metal. Comparisons use medians across interleaved runs plus the four-of-five pair rule, which needs no distribution assumption. An inconclusive run (noise above 2%) (target) re-runs once, then opens an issue; the statistics tool awaits a catalog row (OQ-performance-budgets-and-benchmarking-1).
...
| Throughput stops scaling with cores | `ruralz_filter_duration_seconds` at 32 P | Mutex and block profiles | Shared shards, pools or counters |
...
| Load | Tool, version, command line, offered and achieved rate, connections, key cardinality and distribution |
...
| OQ-performance-budgets-and-benchmarking-3 | Does Testing adopt the interleaved re-run baseline, the Latency job rotation and F2 in the Scale job? | (a) All three (proposed); (b) previous nightly result, S1 and S2 only | testing-and-quality-strategy | Yes, for the M2 F2 gate |
...
| OQ-performance-budgets-and-benchmarking-5 | Tech stack and Testing state a 96 MiB idle RSS gate; this document sets 92 MiB (target) to fit the 100 MB seed. Do they align? | (a) Align to 92 MiB (proposed); (b) restate the seed in binary units | tech-stack-and-libraries | No |
...
| OQ-performance-budgets-and-benchmarking-10 | Sharded label sets cost about 2 KiB per Route at 8 stripes (hypothesis); should the per-Route budget scale with stripes? | (a) No, the base holds 4 stripes (current); (b) a per-stripe allowance | observability | No |
| OQ-performance-budgets-and-benchmarking-11 | How does PB-11 change with load shedding (OQ-data-plane-6), given about 0.54 s of queueing in O1b (hypothesis)? | (a) Fixed in-flight ceiling (current); (b) revise with an adaptive limiter | data-plane | No |
